...
class VisIBM():
    ...
    def ibm_gc_2D(self):
        ...
        ax = plt.axes()
        ax.scatter(fluid[:, 0], fluid[:, 2], c='aqua', s=1, alpha=0.5)
        ax.scatter(solid[:, 0], solid[:, 2], c='darkgrey', s=1)
        ax.scatter(gc[:, 0], gc[:, 2], c='r', s=5, marker='*')
        ax.scatter(ibn[:, 0], ibn[:, 2], c='b', s=2, alpha=0.5)
        ax.set_xlim(-1,3)
        ax.set_ylim(-0.4, -0.2)
        plt.show()
